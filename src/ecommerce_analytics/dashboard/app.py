...
logger.info(f"🎨 Dashboard iniciando...")
        ...
        dcc.Tab(label="💡 Recomendações", value="tab-5", children=[
            html.Div([
                html.Div([
                    ...
                    html.Label("Top N:", style={"fontWeight": "bold", "marginLeft": "20px"}),
                    dcc.Slider(
...
@callback(
    ...
    [Input("dashboard-data-store", "data"), Input("forecast-months", "value"), Input("auth-token-store", "data")],
)
...
@callback(
    ...
    [Input("customer-id-input", "value"), Input("recommendations-limit", "value"), Input("auth-token-store", "data")],
)

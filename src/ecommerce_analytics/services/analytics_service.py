...
class AnalyticsService:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        recent_trends = daily_trends[-30:] if daily_trends is not None and len(daily_trends) > 0 else []
        ...
        for _, p_row in top_products_df.iterrows():
             ...
             top_products_list.append({
                 "name": name, 
                 "sales": float(p_row.get("sales") or 0)
             })
        ...
        for _, cat_row in categories_df.iterrows():
             ...
             categories_list.append({
                 "category": cat_name, 
                 "sales": float(cat_row.get("sales") or 0)
             })

        # Mapeamento do dicionário final para a rota
        avg_ov = float(row.get("avg_order_value", 0)) if pd.notna(row.get("avg_order_value", 0)) else 0.0
